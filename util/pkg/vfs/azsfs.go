...
func (p *AZSPath) Join(relativePath ...string) Path {
	...
	// We are making a huge assumption that the
	// first element in relativePath is the
	// cluster name ie the container name
	// containerNameOrg := relativePath[0]

	// // Azure doesn't like '.' in container names
	// // TODO: BP Is there a better way to do this?
	// containerName := strings.Replace(containerNameOrg, ".", "-", -1)

	// // TODO: BP Is there a better way to set container name?
	// if p.containerName == "" {
	// 	// If we got here then in theory this is the
	// 	// first Join call from the 'find' method in cluster.go
	// 	p.containerName = containerName
	// }

	// Remove container name prefix
	// joined = strings.TrimPrefix(joined, containerNameOrg)
